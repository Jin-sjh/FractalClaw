...
class FractalClawApp:
    ...
    async def process_user_input(self, user_input: str) -> AgentResult:
        ...
        t0 = time.time()
        intent_result = await self._analyze_intent_with_confirmation(context_aware_input)
    ...
    def _build_context_aware_input(self, user_input: str) -> str:
        ...
        context_parts = []
        context_parts.append("[对话历史上下文]")
        for i, turn in enumerate(self._conversation_history[-6:]):
        ...
        if self._last_result:
            status = "成功" if self._last_result.success else f"失败({self._last_result.error})"
            last_result_info = f"\n[上一轮任务执行结果: {status}]"

        context_parts.append(last_result_info)
    ...
    async def _confirm_execution(self) -> bool:
        ...
        while True:
            ...
            if response.lower() in ['y', 'yes', '是', '确认', '开始', '执行']:
                ...
                cprint(f"  \033[38;5;214m任务已取消。\033[0m")
                return False
        ...
        async def agent_worker():
            while True:
                ...
                try:
                    ...
                    self._last_task_text = user_input
                    ...
                    if result.success:
                        ...
                        cprint(f"  \033[38;5;51m输入下一条指令继续，或 /new 开启新会话\033[0m")
                        cprint(f"  \033[38;5;242m───────────────────────────────────────────────\033[0m\n")
                    else:
                        cprint(f"  \033[31m❯ 任务执行失败: {result.error or '未知错误'}\033[0m")
                        cprint(f"\n  \033[38;5;242m可输入修正指令重试，或 /new 开启新会话\033[0m\n")
