...
class AgentConfigGenerator:
    ...
    def generate_from_requirement(
        ...
        if use_ai_generation and self._llm_provider:
            analysis = self._analyze_requirement_with_llm(requirement)
        else:
    ...
    def _select_model_for_role(
        ...
        优先使用 .env 中配置的默认模型，如果没有则自动选择
        """
        
        default_config = self._get_default_model_config()
        if default_config:
            from ..llm.model_profile import ModelProfile, ModelTag, PricingInfo, ModelCapabilities
    ...
    def _get_task_type_for_role(self, role: str) -> TaskType:
        """根据角色获取任务类型"""
        role_task_mapping = {
            "root": TaskType.COORDINATE,
            "coordinator": TaskType.COORDINATE,
            "worker": TaskType.GENERAL,
            "specialist": TaskType.GENERAL
        }
        return role_task_mapping.get(role, TaskType.GENERAL)
    
    def _get_default_model_config(self) -> Optional[dict[str, Any]]:
        """从 .env 读取默认模型配置
        ...
        base_url = os.getenv("OPENAI_API_BASE") or os.getenv("ANTHROPIC_BASE_URL")
        ...
        config["selection_reason"] = f"使用用户配置的默认模型: {default_model} (provider: {default_provider or 'openai'})"
    ...
    def _build_llm_config_from_selection(self, selection: SelectionResult) -> dict[str, Any]:
        ...
        env_base_url = os.getenv("OPENAI_API_BASE") or os.getenv("ANTHROPIC_BASE_URL")
    ...
    def _analyze_requirement(self, requirement: str) -> dict[str, Any]:
        ...
        keywords = {
            "code": ["代码", "编程", "开发", "coder", "code", "program", "编写代码", "代码审查"],
            "research": ["研究", "搜索", "分析", "research", "search", "analyze", "调研", "报告"],
            "coordinate": ["协调", "管理", "分配", "coordinate", "manage", "orchestrate", "统筹"],
            "test": ["测试", "验证", "test", "verify", "单元测试", "集成测试"],
            "data": ["数据", "处理", "data", "process", "数据分析", "数据处理"],
            "chat": ["对话", "聊天", "chat", "conversation", "问答"]
        }
        ...
        for category, kws in keywords.items():
            for kw in kws:
                if kw in requirement_lower:
                    ...
                    if category == "code":
                        role = "specialist"
                        tools.extend(["read_file", "write_file", "execute_code"])
                        capabilities.extend(["代码编写", "代码审查", "调试排错"])
                    elif category == "research":
                        role = "specialist"
                        tools.extend(["web_search", "summarize", "analyze_data"])
                        capabilities.extend(["信息搜索", "数据分析", "报告撰写"])
                    elif category == "coordinate":
                        role = "coordinator"
                        capabilities.extend(["任务分解", "资源协调", "结果整合"])
                    elif category == "test":
                        role = "specialist"
                        tools.extend(["read_file", "execute_code"])
                        capabilities.extend(["测试编写", "测试执行", "问题报告"])
                    elif category == "data":
                        role = "specialist"
                        tools.extend(["analyze_data", "summarize"])
                        capabilities.extend(["数据处理", "数据分析", "可视化"])
                    elif category == "chat":
                        role = "worker"
                        capabilities.extend(["对话理解", "信息提供", "问题解答"])
                    break
        ...
        requires_code = task_type == "code" or "代码" in requirement
        requires_reasoning = task_type in ["research", "coordinate"] or "推理" in requirement
    ...
    async def _analyze_requirement_with_llm(self, requirement: str) -> dict[str, Any]:
        """使用 LLM 分析需求描述，生成 name、description、role"""
...
请以 JSON 格式返回以下信息：
{{
    "name": "Agent的英文名称（简洁，驼峰命名，如 CodeAgent）",
    "description": "一句话描述 Agent 的功能",
    "role": "角色类型（worker/specialist/coordinator 之一）",
    "capabilities": ["核心能力1", "核心能力2"],
    "task_type": "任务类型（code/research/coordinate/test/data/chat/general 之一）",
...
只返回 JSON，不要其他内容。"""
        ...
        try:
            ...
            config = LLMConfig(model="gpt-4", temperature=0.3, max_tokens=1024, stream=False)
            ...
            result.setdefault("requires_reasoning", result.get("task_type") in ["research", "coordinate"])
...
直接返回提示词内容，不要包含其他说明。"""
        ...
        try:
            ...
            config = LLMConfig(model="gpt-4", temperature=0.7, max_tokens=2048, stream=False)
            messages = [Message(role=MessageRole.USER, content=prompt)]
